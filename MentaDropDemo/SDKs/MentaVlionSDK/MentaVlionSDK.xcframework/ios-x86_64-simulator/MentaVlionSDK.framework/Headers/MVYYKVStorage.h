//
//  MVYYKVStorage.h
//  MVYYCache <https://github.com/ibireme/MVYYCache>
//
...
/**
 MVYYKVStorageItem is used by `MVYYKVStorage` to store key-value pair and meta data.
 Typically, you should not use this class directly.
 */
@interface MVYYKVStorageItem : NSObject
...
/**
 Storage type, indicated where the `MVYYKVStorageItem.value` stored.
 ...
 * If you want to store large number of small datas (such as contacts cache), 
   use MVYYKVStorageTypeSQLite to get better performance.
 * If you want to store large files (such as image cache),
   use MVYYKVStorageTypeFile to get better performance.
 * You can use MVYYKVStorageTypeMixed and choice your storage type for each item.
 
 See <http://www.sqlite.org/intern-v-extern-blob.html> for more information.
 */
typedef NS_ENUM(NSUInteger, MVYYKVStorageType) {
    
    /// The `value` is stored as a file in file system.
    MVYYKVStorageTypeFile = 0,
    
    /// The `value` is stored in sqlite with blob type.
    MVYYKVStorageTypeSQLite = 1,
    
    /// The `value` is stored in file system or sqlite based on your choice.
    MVYYKVStorageTypeMixed = 2,
};



/**
 MVYYKVStorage is a key-value storage based on sqlite and file system.
 Typically, you should not use this class directly.
 
 @discussion The designated initializer for MVYYKVStorage is `initWithPath:type:`. 
 ...
 */
@interface MVYYKVStorage : NSObject
...
@property (nonatomic, readonly) MVYYKVStorageType type;  ///< The type of this storage.
 ...
 */
- (nullable instancetype)initWithPath:(NSString *)path type:(MVYYKVStorageType)type NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 If the `type` is MVYYKVStorageTypeFile, then the item.filename should not be empty.
 If the `type` is MVYYKVStorageTypeSQLite, then the item.filename will be ignored.
 It the `type` is MVYYKVStorageTypeMixed, then the item.value will be saved to file 
 system if the item.filename is not empty, otherwise it will be saved to sqlite.
 
 @param item  An item.
 @return Whether succeed.
 */
- (BOOL)saveItem:(MVYYKVStorageItem *)item;

/**
 Save an item or update the item with 'key' if it already exists.
 
 @discussion This method will save the key-value pair to sqlite. If the `type` is
 MVYYKVStorageTypeFile, then this method will failed.
...
/**
 ...
 @discussion
 If the `type` is MVYYKVStorageTypeFile, then the `filename` should not be empty.
 If the `type` is MVYYKVStorageTypeSQLite, then the `filename` will be ignored.
 It the `type` is MVYYKVStorageTypeMixed, then the `value` will be saved to file
 ...
 */
- (nullable MVYYKVStorageItem *)getItemForKey:(NSString *)key;
 ...
 */
- (nullable MVYYKVStorageItem *)getItemInfoForKey:(NSString *)key;
...
/**
 ...
 @param keys  An array of specified keys.
 @return An array of `MVYYKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<MVYYKVStorageItem *> *)getItemForKeys:(NSArray<NSString *> *)keys;
...
/**
 ...
 @return An array of `MVYYKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<MVYYKVStorageItem *> *)getItemInfoForKeys:(NSArray<NSString *> *)keys;
