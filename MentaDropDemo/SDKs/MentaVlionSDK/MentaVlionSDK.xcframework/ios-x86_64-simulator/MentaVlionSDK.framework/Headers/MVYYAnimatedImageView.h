...
/**
 ...
 If the `image` or `highlightedImage` property adopt to the `MVYYAnimatedImage` protocol,
...
/**
 The MVYYAnimatedImage protocol declares the required methods for animated image
 display with MVYYAnimatedImageView.
 
 Subclass a UIImage and implement this protocol, so that instances of that class 
 can be set to MVYYAnimatedImageView.image or MVYYAnimatedImageView.highlightedImage
 to display animation.
 
 See `MVYYImage` and `MVYYFrameImage` for example.
 */
@protocol MVYYAnimatedImage <NSObject>
@required
