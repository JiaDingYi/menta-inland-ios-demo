...
@class MVYYMemoryCache, MVYYDiskCache;

NS_ASSUME_NONNULL_BEGIN

/// Image cache type
typedef NS_OPTIONS(NSUInteger, MVYYImageCacheType) {
    /// No value.
    MVYYImageCacheTypeNone   = 0,
    
    /// Get/store image with memory cache.
    MVYYImageCacheTypeMemory = 1 << 0,
    
    /// Get/store image with disk cache.
    MVYYImageCacheTypeDisk   = 1 << 1,
    
    /// Get/store image with both memory cache and disk cache.
    MVYYImageCacheTypeAll    = MVYYImageCacheTypeMemory | MVYYImageCacheTypeDisk,
};


/**
 MVYYImageCache is a cache that stores UIImage and image data based on memory cache and disk cache.
 ...
 */
@interface MVYYImageCache : NSObject
...
/** The underlying memory cache. see `MVYYMemoryCache` for more information.*/
@property (strong, readonly) MVYYMemoryCache *memoryCache;

/** The underlying disk cache. see `MVYYDiskCache` for more information.*/
@property (strong, readonly) MVYYDiskCache *diskCache;

/**
 Whether decode animated image when fetch image from disk cache. Default is YES.
 
 @discussion When fetch image from disk cache, it will use 'MVYYImage' to decode 
...
/**
 ...
 @return  The singleton MVYYImageCache instance.
 */
...
/**
 ...
 @discussion If the `type` contain `MVYYImageCacheTypeMemory`, then the `image` will 
 be stored in the memory cache; `imageData` will be used instead if `image` is nil.
 If the `type` contain `MVYYImageCacheTypeDisk`, then the `imageData` will
...
- (void)setImage:(nullable UIImage *)image
       imageData:(nullable NSData *)imageData
          forKey:(NSString *)key
        withType:(MVYYImageCacheType)type;
 ...
 */
- (void)removeImageForKey:(NSString *)key withType:(MVYYImageCacheType)type;
...
/**
 Returns a Boolean value that indicates whether a given key is in cache.
 If the image is not in memory and the `type` contains `MVYYImageCacheTypeDisk`,
 ...
 @return Whether the image is in cache.
 */
- (BOOL)containsImageForKey:(NSString *)key withType:(MVYYImageCacheType)type;
...
/**
 ...
 @return The image associated with key, or nil if no image is associated with key.
 */
- (nullable UIImage *)getImageForKey:(NSString *)key withType:(MVYYImageCacheType)type;
...
- (void)getImageForKey:(NSString *)key
              withType:(MVYYImageCacheType)type
             withBlock:(void(^)(UIImage * _Nullable image, MVYYImageCacheType type))block;
